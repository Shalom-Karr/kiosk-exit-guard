...
Background auto-update check + interactive admin notification. The controller now polls GitHub's `/releases/latest` on startup (after a 60 s settle delay) and every 24 h thereafter; when a newer version is published it spawns a branded WebView2 modal — *"An SK Filter update was pushed. Update to v{newver} now?"* — with [Update Now] / [Remind Me Later] buttons. Update Now hands off to the existing `--update` flow (password modal, SHA-256 verify, atomic swap, Service restart) so auto-update never installs silently; the admin password is still the confirmation of intent. Remind Me Later (or 60 s auto-dismiss, or window close) defers to the next 24 h tick. A new `Global\KioskExitGuardAutoUpdateNotify` admin-only mutex prevents two stacked notify modals if a previous one is still on-screen when the next tick fires. The checker only runs in the long-lived controller process — every short-lived flag invocation (`--pause`, `--update`, `--silent-exit`, etc.) returns earlier in `main()` without reaching the goroutine launch, so a desktop-shortcut click won't accidentally hammer GitHub.

## What's in v1.2.0

Consolidation release combining the v1.1.5–v1.1.11 work plus an audit pass that fixed one CRITICAL `WTS_PROCESS_INFOW` struct-layout bug, three HIGH install/update issues (default-No on missing SHA-256 sidecar, `Global\KioskExitGuardFirstRunRelocate` mutex against double-click races, env-strip on the relocate re-exec), four MEDIUM items (password-modal timer armed in `kgReady`, multi-RDP-user visibility, update mutex, go-vet), and two LOW items (atomic `MoveFileEx(MOVEFILE_REPLACE_EXISTING)` install swap, uninstall allowlist). All three named mutexes (controller, first-run, update, plus v1.2.1's auto-update-notify) share an `acquireAdminOnlyNamedMutex` helper using SDDL `D:(A;;0x1F0001;;;BA)(A;;0x1F0001;;;SY)` so a kiosk-user logon script can't pre-create one and DoS the legitimate controller.
